...
from avocado.rpc.full_node_rpc_api import FullNodeRpcApi
...
class TestWalletRpc:
    ...
    async def test_wallet_make_transaction(self, two_wallet_nodes):
        ...
        try:
            addr = encode_puzzle_hash(await wallet_node_2.wallet_state_manager.main_wallet.get_new_puzzlehash(), "avo")
            tx_amount = 15600000
            try:
            ...
            for i in range(0, 5):
                await client.farm_block(encode_puzzle_hash(ph_2, "avo"))
            ...
            test_config["farmer"]["avo_target_address"] = encode_puzzle_hash(test_ph, "tavo")
            # set pool to second private key
            sk = wallet_node.get_key_for_fingerprint(pks[1])
            test_ph = create_puzzlehash_for_pk(master_sk_to_wallet_sk(sk, uint32(0)).get_g1())
            test_config["pool"]["avo_target_address"] = encode_puzzle_hash(test_ph, "tavo")
