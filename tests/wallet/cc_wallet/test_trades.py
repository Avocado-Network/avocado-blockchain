...
class TestCCTrades:
    ...
    async def test_cc_trade_cancel_insecure(self, wallets_prefarm):
        ...
        spendable_avocado = await wallet_a.get_spendable_balance()

        offer_dict = {1: 10, 2: -30, 3: 30}

        success, trade_offer, error = await trade_manager_a.create_offer_for_ids(offer_dict, file)

        spendable_avocado_after = await wallet_a.get_spendable_balance()
        ...
        assert spendable_avocado == spendable_avocado_after + locked_sum
        assert success is True
        assert trade_offer is not None
        ...
        assert spendable_avocado == spendable_after_cancel_1
    ...
    async def test_cc_trade_cancel_secure(self, wallets_prefarm):
        ...
        await time_out_assert(15, wallet_a.get_spendable_balance, spendable_avocado)
