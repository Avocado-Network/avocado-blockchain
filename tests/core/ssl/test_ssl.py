...
from avocado.protocols.shared_protocol import protocol_version
from avocado.server.outbound_message import NodeType
from avocado.server.server import AvocadoServer, ssl_context_for_client
from avocado.server.ws_connection import WSAvocadoConnection
from avocado.ssl.create_ssl import generate_ca_signed_cert
from avocado.types.peer_info import PeerInfo
from tests.block_tools import test_constants
from avocado.util.ints import uint16
...
async def establish_connection(server: AvocadoServer, dummy_port: int, ssl_context) -> bool:
...
class TestSSL:
    ...
    async def test_public_connections(self, wallet_node):
        ...
        server_1: AvocadoServer = full_node_api.full_node.server
    ...
    async def test_farmer(self, harvester_farmer):
        ...
        generate_ca_signed_cert(
            farmer_server.avocado_ca_crt_path.read_bytes(), farmer_server.avocado_ca_key_path.read_bytes(), pub_crt, pub_key
        )
        ssl_context = ssl_context_for_client(
            farmer_server.avocado_ca_crt_path, farmer_server.avocado_ca_crt_path, pub_crt, pub_key
        )
        connected = await establish_connection(farmer_server, 12312, ssl_context)
    ...
    async def test_full_node(self, wallet_node):
        ...
        generate_ca_signed_cert(
            full_node_server.avocado_ca_crt_path.read_bytes(),
            full_node_server.avocado_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            full_node_server.avocado_ca_crt_path, full_node_server.avocado_ca_crt_path, pub_crt, pub_key
        )
    ...
    async def test_wallet(self, wallet_node):
        ...
        generate_ca_signed_cert(
            wallet_server.avocado_ca_crt_path.read_bytes(), wallet_server.avocado_ca_key_path.read_bytes(), pub_crt, pub_key
        )
        ssl_context = ssl_context_for_client(
            wallet_server.avocado_ca_crt_path, wallet_server.avocado_ca_crt_path, pub_crt, pub_key
        )
    ...
    async def test_harvester(self, harvester_farmer):
        ...
        generate_ca_signed_cert(
            harvester_server.avocado_ca_crt_path.read_bytes(),
            harvester_server.avocado_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            harvester_server.avocado_ca_crt_path, harvester_server.avocado_ca_crt_path, pub_crt, pub_key
        )
    ...
    async def test_introducer(self, introducer):
        ...
        # Create not authenticated cert
        pub_crt = introducer_server.avocado_ca_key_path.parent / "p2p.crt"
        pub_key = introducer_server.avocado_ca_key_path.parent / "p2p.key"
        generate_ca_signed_cert(
            introducer_server.avocado_ca_crt_path.read_bytes(),
            introducer_server.avocado_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            introducer_server.avocado_ca_crt_path, introducer_server.avocado_ca_crt_path, pub_crt, pub_key
        )
    ...
    async def test_timelord(self, timelord):
        ...
        generate_ca_signed_cert(
            timelord_server.avocado_ca_crt_path.read_bytes(),
            timelord_server.avocado_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            timelord_server.avocado_ca_crt_path, timelord_server.avocado_ca_crt_path, pub_crt, pub_key
        )
