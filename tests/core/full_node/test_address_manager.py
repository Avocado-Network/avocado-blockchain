...
class TestPeerManager:
    ...
    async def test_addr_manager(self):
        ...
        peer1_duplicate = PeerInfo("250.1.1.1", 7865)
    ...
    async def test_addr_manager_ports(self):
        addrman = AddressManagerTest()
        assert await addrman.size() == 0
        source = PeerInfo("252.2.2.2", 7865)
    ...
    async def test_addrman_select(self):
        ...
        peer2 = PeerInfo("250.3.1.1", 7865)
        peer3 = PeerInfo("250.3.2.2", 9999)
        peer4 = PeerInfo("250.3.3.3", 9999)

        assert await addrman.add_peer_info([peer2], PeerInfo("250.3.1.1", 7865))
        assert await addrman.add_peer_info([peer3], PeerInfo("250.3.1.1", 7865))
        assert await addrman.add_peer_info([peer4], PeerInfo("250.4.1.1", 7865))

        # Add three addresses to tried table.
        peer5 = PeerInfo("250.4.4.4", 7865)
        peer6 = PeerInfo("250.4.5.5", 7777)
        peer7 = PeerInfo("250.4.6.6", 7865)

        assert await addrman.add_peer_info([peer5], PeerInfo("250.3.1.1", 7865))
        await addrman.mark_good(peer5)
        assert await addrman.add_peer_info([peer6], PeerInfo("250.3.1.1", 7865))
        await addrman.mark_good(peer6)
        assert await addrman.add_peer_info([peer7], PeerInfo("250.1.1.3", 7865))
    ...
    async def test_addrman_collisions_new(self):
        ...
        for i in range(1, 8):
            peer = PeerInfo("250.1.1." + str(i), 7865)
            assert await addrman.add_peer_info([peer], source)
            assert await addrman.size() == i

        # Test: new table collision!
        peer1 = PeerInfo("250.1.1.8", 7865)
        assert await addrman.add_peer_info([peer1], source)
        assert await addrman.size() == 7

        peer2 = PeerInfo("250.1.1.9", 7865)
    ...
    async def test_addrman_collisions_tried(self):
        ...
        peer1 = PeerInfo("250.1.1.77", 7865)
        assert await addrman.add_peer_info([peer1], source)
        assert await addrman.size() == 76

        peer2 = PeerInfo("250.1.1.78", 7865)
        assert await addrman.add_peer_info([peer2], source)
    ...
    async def test_addrman_find(self):
        ...
        source1 = PeerInfo("250.1.2.1", 7865)
        source2 = PeerInfo("250.1.2.2", 7865)
    ...
    async def test_addrman_create(self):
        ...
        peer1 = PeerInfo("250.1.2.1", 7865)
        t_peer = TimestampedPeerInfo("250.1.2.1", 7865, 0)
    ...
    async def test_addrman_get_peers(self):
        ...
        peer1 = TimestampedPeerInfo("250.250.2.1", 7865, time.time())
        peer2 = TimestampedPeerInfo("250.250.2.2", 9999, time.time())
        peer3 = TimestampedPeerInfo("251.252.2.3", 7865, time.time())
        peer4 = TimestampedPeerInfo("251.252.2.4", 7865, time.time())
        peer5 = TimestampedPeerInfo("251.252.2.5", 7865, time.time())
        source1 = PeerInfo("250.1.2.1", 7865)
        source2 = PeerInfo("250.2.3.3", 7865)
        ...
        for i in range(1, 8 * 256):
            octet1 = i % 256
            octet2 = i >> 8 % 256
            peer = TimestampedPeerInfo(str(octet1) + "." + str(octet2) + ".1.23", 7865, time.time())
    ...
    async def test_addrman_tried_bucket(self):
        peer1 = PeerInfo("250.1.1.1", 7865)
        t_peer1 = TimestampedPeerInfo("250.1.1.1", 7865, 0)
        peer2 = PeerInfo("250.1.1.1", 9999)
        t_peer2 = TimestampedPeerInfo("250.1.1.1", 9999, 0)
        source1 = PeerInfo("250.1.1.1", 7865)
        ...
        for i in range(255):
            peer = PeerInfo("250.1.1." + str(i), 7865)
            t_peer = TimestampedPeerInfo("250.1.1." + str(i), 7865, 0)
        ...
        buckets = []
        for i in range(255):
            peer = PeerInfo("250." + str(i) + ".1.1", 7865)
            t_peer = TimestampedPeerInfo("250." + str(i) + ".1.1", 7865, 0)
            extended_peer_info = ExtendedPeerInfo(t_peer, peer)
            bucket = extended_peer_info.get_tried_bucket(key1)
            if bucket not in buckets:
                buckets.append(bucket)
    ...
    async def test_addrman_new_bucket(self):
        t_peer1 = TimestampedPeerInfo("250.1.2.1", 7865, 0)
        source1 = PeerInfo("250.1.2.1", 7865)
        ...
        for i in range(4 * 255):
            src = PeerInfo("251.4.1.1", 7865)
            peer = PeerInfo(str(250 + i // 255) + "." + str(i % 256) + ".1.1", 7865)
            t_peer = TimestampedPeerInfo(str(250 + i // 255) + "." + str(i % 256) + ".1.1", 7865, 0)
        ...
        buckets = []
        for i in range(255):
            src = PeerInfo("250." + str(i) + ".1.1", 7865)
            peer = PeerInfo("250.1.1.1", 7865)
            t_peer = TimestampedPeerInfo("250.1.1.1", 7865, 0)
            extended_peer_info = ExtendedPeerInfo(t_peer, src)
            bucket = extended_peer_info.get_new_bucket(key1)
            if bucket not in buckets:
                buckets.append(bucket)
    ...
    async def test_addrman_no_evict(self):
        ...
        # Cause a collision.
        peer37 = PeerInfo("250.1.1.37", 7865)
    ...
    async def test_addrman_eviction_works(self):
        ...
        addr16 = PeerInfo("250.1.1.16", 7865)
        assert not await addrman.add_peer_info([addr16], source)
        await addrman.mark_good(addr16)
        collision = await addrman.select_tried_collision()
        assert collision.peer_info == PeerInfo("250.1.1.18", 7865)
        await addrman.resolve_tried_collisions()
        assert await addrman.select_tried_collision() is None
    ...
    async def test_cleanup(self):
        addrman = AddressManagerTest()
        peer1 = TimestampedPeerInfo("250.250.2.1", 7865, 100000)
        peer2 = TimestampedPeerInfo("250.250.2.2", 9999, time.time())
        source = PeerInfo("252.5.1.1", 8333)
