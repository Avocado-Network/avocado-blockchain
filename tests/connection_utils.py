...
async def disconnect_all_and_reconnect(server: AvocadoServer, reconnect_to: AvocadoServer) -> bool:
...
async def add_dummy_connection(server: AvocadoServer, dummy_port: int) -> Tuple[asyncio.Queue, bytes32]:
    ...
    generate_ca_signed_cert(
        server.avocado_ca_crt_path.read_bytes(), server.avocado_ca_key_path.read_bytes(), dummy_crt_path, dummy_key_path
    )
    ssl_context = ssl_context_for_client(
        server.avocado_ca_crt_path, server.avocado_ca_key_path, dummy_crt_path, dummy_key_path
    )
...
async def connect_and_get_peer(server_1: AvocadoServer, server_2: AvocadoServer) -> WSAvocadoConnection:
    """
