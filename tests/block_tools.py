...
from avocado.cmds.init_funcs import create_all_ssl, create_default_avocado_config
from avocado.full_node.bundle_tools import (
    best_solution_generator_from_template,
    detect_potential_template_generator,
    simple_solution_generator,
)
from avocado.util.errors import Err
from avocado.full_node.generator import setup_generator_args
from avocado.full_node.mempool_check_conditions import GENERATOR_MOD
...
from avocado.consensus.blockchain_interface import BlockchainInterface
from avocado.consensus.coinbase import create_puzzlehash_for_pk, create_farmer_coin, create_pool_coin
...
from avocado.consensus.deficit import calculate_deficit
from avocado.consensus.full_block_to_block_record import block_to_block_record
from avocado.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from avocado.consensus.cost_calculator import NPCResult, calculate_cost_of_program
from avocado.consensus.pot_iterations import (
    calculate_ip_iters,
...
from avocado.consensus.vdf_info_computation import get_signage_point_vdf_info
from avocado.full_node.signage_point import SignagePoint
from avocado.plotting.plot_tools import PlotInfo, load_plots, parse_plot_info
...
from avocado.types.blockchain_format.foliage import Foliage, FoliageBlockData, FoliageTransactionBlock, TransactionsInfo
...
from avocado.types.blockchain_format.reward_chain_block import RewardChainBlockUnfinished
...
from avocado.types.generator_types import BlockGenerator, CompressorArg
from avocado.types.spend_bundle import SpendBundle
from avocado.types.unfinished_block import UnfinishedBlock
from avocado.types.name_puzzle_condition import NPC
from avocado.util.bech32m import encode_puzzle_hash
from avocado.util.block_cache import BlockCache
from avocado.util.condition_tools import ConditionOpcode, conditions_by_opcode
from avocado.util.config import load_config, save_config
from avocado.util.hash import std_hash
from avocado.util.ints import uint8, uint16, uint32, uint64, uint128
from avocado.util.keychain import Keychain, bytes_to_mnemonic
from avocado.util.merkle_set import MerkleSet
from avocado.util.prev_transaction_block import get_prev_transaction_block
from avocado.util.path import mkdir
from avocado.util.vdf_prover import get_vdf_info_and_proof
...
class BlockTools:
    ...
    def __init__(
        ...
        create_default_avocado_config(root_path)
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `avocado generate keys`")
    ...
    def init_plots(self, root_path: Path):
        ...
        try:
            ...
            args.pool_contract_address = encode_puzzle_hash(self.pool_ph, "avo")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("AVOCADO_ROOT", "~/.avocado/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
