...
from avocado.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from avocado.full_node.full_node_api import FullNodeAPI
from avocado.server.start_farmer import service_kwargs_for_farmer
from avocado.server.start_full_node import service_kwargs_for_full_node
from avocado.server.start_harvester import service_kwargs_for_harvester
from avocado.server.start_introducer import service_kwargs_for_introducer
from avocado.server.start_service import Service
from avocado.server.start_timelord import service_kwargs_for_timelord
from avocado.server.start_wallet import service_kwargs_for_wallet
from avocado.simulator.start_simulator import service_kwargs_for_full_node_simulator
from avocado.timelord.timelord_launcher import kill_processes, spawn_process
...
async def setup_farmer(
    ...
    config["avo_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "avo")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["avo_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "avo")
