...
from avocado.plotting.plot_tools import add_plot_directory, stream_plot_info_ph, stream_plot_info_pk
...
from avocado.util.config import config_path_for_filename, load_config
from avocado.util.keychain import Keychain
from avocado.util.path import mkdir
from avocado.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_local_sk, master_sk_to_pool_sk
...
def get_farmer_public_key(alt_fingerprint: Optional[int] = None) -> G1Element:
    ...
    if sk_ent is None:
        raise RuntimeError("No keys, please run 'avocado keys add', 'avocado keys generate' or provide a public key with -f")
...
def get_pool_public_key(alt_fingerprint: Optional[int] = None) -> G1Element:
    ...
    if alt_fingerprint is not None:
        ...
        sk_ent = keychain.get_first_private_key()
    if sk_ent is None:
        raise RuntimeError("No keys, please run 'avocado keys add', 'avocado keys generate' or provide a public key with -p")
