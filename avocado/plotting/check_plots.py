...
from avocado.plotting.plot_tools import find_duplicate_plot_IDs, get_plot_filenames, load_plots, parse_plot_info
...
from avocado.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_local_sk
...
def check_plots(root_path, num, challenge_start, grep_string, list_duplicates, debug_show_memo):
    ...
    if len(no_key_filenames) > 0:
        log.warning(
            ...
            f"farm them, use 'avocado keys' to transfer keys. The pool public keys must be in the config.yaml"
        )
