...
from avocado.cmds.init_funcs import avocado_init
from avocado.daemon.windows_signal import kill
from avocado.server.server import ssl_context_for_root, ssl_context_for_server
from avocado.ssl.create_ssl import get_mozilla_ca_crt
from avocado.util.avocado_logging import initialize_logging
from avocado.util.config import load_config
from avocado.util.json_util import dict_to_json_str
from avocado.util.path import mkdir
from avocado.util.service_groups import validate_service
from avocado.util.setproctitle import setproctitle
from avocado.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Avocado.")
    quit()
...
service_plotter = "avocado plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "avocado": "avocado",
        "avocado_wallet": "start_wallet",
        "avocado_full_node": "start_full_node",
        "avocado_harvester": "start_harvester",
        "avocado_farmer": "start_farmer",
        "avocado_introducer": "start_introducer",
        "avocado_timelord": "start_timelord",
        "avocado_timelord_launcher": "timelord_launcher",
        "avocado_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("avocado daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered AVOCADO_ROOT
    os.environ["AVOCADO_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with AVOCADO_ROOT: {os.environ['AVOCADO_ROOT']}")
...
async def async_run_daemon(root_path: Path) -> int:
    avocado_init(root_path)
    config = load_config(root_path, "config.yaml")
    setproctitle("avocado_daemon")
