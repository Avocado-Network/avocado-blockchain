...
from avocado.wallet.cc_wallet import cc_utils
from avocado.wallet.cc_wallet.cc_utils import CC_MOD, SpendableCC, spend_bundle_for_spendable_ccs, uncurry_cc
from avocado.wallet.cc_wallet.cc_wallet import CCWallet
from avocado.wallet.puzzles.genesis_by_coin_id_with_0 import genesis_coin_id_for_genesis_coin_checker
from avocado.wallet.trade_record import TradeRecord
from avocado.wallet.trading.trade_status import TradeStatus
from avocado.wallet.trading.trade_store import TradeStore
from avocado.wallet.transaction_record import TransactionRecord
from avocado.wallet.util.trade_utils import (
            ...
            for id in offer.keys():
                ...
                if isinstance(wallet, CCWallet):
                    ...
                    if spend_bundle is None:
                        ...
                        to_exclude = spend_bundle.removals()
                    new_spend_bundle = await wallet.create_spend_bundle_relative_avocado(amount, to_exclude)
                else:
    ...
    async def respond_to_offer(self, file_path: Path) -> Tuple[bool, Optional[TradeRecord], Optional[str]]:
        ...
        for coinsol in offer_spend_bundle.coin_solutions:
            ...
            if r:
                ...
                # standard avocado coin
                unspent = await self.wallet_state_manager.get_spendable_coins_for_wallet(1)
                if coinsol.coin in [record.coin for record in unspent]:
                    return False, None, "can't respond to own offer"
                if avocado_discrepancy is None:
                    avocado_discrepancy = get_output_discrepancy_for_puzzle_and_solution(coinsol.coin, puzzle, solution)
                else:
                    avocado_discrepancy += get_output_discrepancy_for_puzzle_and_solution(coinsol.coin, puzzle, solution)
                coinsols.append(coinsol)

        avocado_spend_bundle: Optional[SpendBundle] = None
        if avocado_discrepancy is not None:
            avocado_spend_bundle = await self.wallet_state_manager.main_wallet.create_spend_bundle_relative_avocado(
                avocado_discrepancy, []
            )
            if avocado_spend_bundle is not None:
                for coinsol in coinsols:
                    avocado_spend_bundle.coin_solutions.append(coinsol)
        ...
        for colour in cc_discrepancies.keys():
            if cc_discrepancies[colour] < 0:
                ...
                if avocado_spend_bundle is None:
                    to_exclude: List = []
                else:
                    to_exclude = avocado_spend_bundle.removals()
                ...
                if my_cc_spends is None or my_cc_spends == set():
                    ...
                    if zero_spend_bundle is None:
                        return (
                            False,
                            None,
                            "Unable to generate zero value coin. Confirm that you have avocado available",
                        )
        ...
        now = uint64(int(time.time()))
        if avocado_spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, avocado_spend_bundle])
            if avocado_discrepancy < 0:
                tx_record = TransactionRecord(
                    confirmed_at_height=uint32(0),
                    created_at_time=now,
                    to_puzzle_hash=token_bytes(),
                    amount=uint64(abs(avocado_discrepancy)),
                    fee_amount=uint64(0),
                    confirmed=False,
                    sent=uint32(10),
                    spend_bundle=avocado_spend_bundle,
                    additions=avocado_spend_bundle.additions(),
                    removals=avocado_spend_bundle.removals(),
                    wallet_id=uint32(1),
                    sent_to=[],
                    trade_id=std_hash(spend_bundle.name() + bytes(now)),
                    type=uint32(TransactionType.OUTGOING_TRADE.value),
                    name=avocado_spend_bundle.name(),
                )
            else:
                tx_record = TransactionRecord(
                    confirmed_at_height=uint32(0),
