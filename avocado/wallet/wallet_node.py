...
from avocado.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from avocado.protocols import wallet_protocol
from avocado.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from avocado.protocols.protocol_message_types import ProtocolMessageTypes
from avocado.protocols.wallet_protocol import (
...
from avocado.server.node_discovery import WalletPeers
...
from avocado.util.errors import Err, ValidationError
from avocado.util.ints import uint32, uint128
from avocado.util.keychain import Keychain
from avocado.util.lru_cache import LRUCache
...
from avocado.wallet.settings.settings_objects import BackupInitialized
...
from avocado.wallet.wallet_blockchain import ReceiveBlockResult
from avocado.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'avocado keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSAvocadoConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSAvocadoConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSAvocadoConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSAvocadoConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSAvocadoConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSAvocadoConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
