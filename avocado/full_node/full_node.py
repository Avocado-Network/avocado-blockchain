...
import avocado.server.ws_connection as ws  # lgtm [py/import-and-import-from]
from avocado.consensus.block_creation import unfinished_block_to_full_block
from avocado.consensus.block_record import BlockRecord
...
from avocado.consensus.difficulty_adjustment import get_next_sub_slot_iters_and_difficulty
from avocado.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from avocado.consensus.multiprocess_validation import PreValidationResult
from avocado.consensus.pot_iterations import calculate_sp_iters
from avocado.full_node.block_store import BlockStore
from avocado.full_node.bundle_tools import detect_potential_template_generator
from avocado.full_node.coin_store import CoinStore
from avocado.full_node.full_node_store import FullNodeStore
from avocado.full_node.mempool_manager import MempoolManager
from avocado.full_node.signage_point import SignagePoint
from avocado.full_node.sync_store import SyncStore
from avocado.full_node.weight_proof import WeightProofHandler
from avocado.protocols import farmer_protocol, full_node_protocol, timelord_protocol, wallet_protocol
from avocado.protocols.full_node_protocol import (
    RejectBlocks,
    RequestBlocks,
    RespondBlock,
    RespondBlocks,
...
from avocado.server.node_discovery import FullNodePeers
from avocado.server.outbound_message import Message, NodeType, make_msg
from avocado.server.server import AvocadoServer
from avocado.types.blockchain_format.classgroup import ClassgroupElement
...
from avocado.types.blockchain_format.vdf import CompressibleVDFField, VDFInfo, VDFProof
from avocado.types.end_of_slot_bundle import EndOfSubSlotBundle
from avocado.types.full_block import FullBlock
from avocado.types.header_block import HeaderBlock
...
from avocado.types.unfinished_block import UnfinishedBlock
from avocado.util.bech32m import encode_puzzle_hash
from avocado.util.db_wrapper import DBWrapper
from avocado.util.errors import ConsensusError, Err
from avocado.util.ints import uint8, uint32, uint64, uint128
from avocado.util.path import mkdir, path_from_root
from avocado.util.safe_cancel_task import cancel_task_safe
from avocado.util.profiler import profile_task
...
class FullNode:
    ...
    def set_server(self, server: AvocadoServer):
        self.server = server
        dns_servers = []
        try:
        ...
        elif self.config["port"] == 7865:
            # If `dns_servers` misses from the `config`, hardcode it if we're running mainnet.
            dns_servers.append("dns-introducer.avocadonetwork.net")
        try:
    ...
    async def short_sync_batch(self, peer: ws.WSAvocadoConnection, start_height: uint32, target_height: uint32) -> bool:
        """
    ...
    async def short_sync_backtrack(
        self, peer: ws.WSAvocadoConnection, peak_height: uint32, target_height: uint32, target_unf_hash: bytes32
    ):
        """
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: ws.WSAvocadoConnection):
        """
    ...
    async def send_peak_to_timelords(
        self, peak_block: Optional[FullBlock] = None, peer: Optional[ws.WSAvocadoConnection] = None
    ):
        """
    ...
    async def on_connect(self, connection: ws.WSAvocadoConnection):
        """
    ...
    def on_disconnect(self, connection: ws.WSAvocadoConnection):
    ...
    async def peak_post_processing(
        self, block: FullBlock, record: BlockRecord, fork_height: uint32, peer: Optional[ws.WSAvocadoConnection]
    ):
        """
    ...
    async def respond_block(
        ...
        peer: Optional[ws.WSAvocadoConnection] = None,
    ) -> Optional[Message]:
        """
    ...
    async def respond_unfinished_block(
        ...
        peer: Optional[ws.WSAvocadoConnection],
        ...
        if farmed_block is True:
            ...
            self.log.info(
                ...
                f"Pool pk {encode_puzzle_hash(block.foliage.foliage_block_data.pool_target.puzzle_hash, 'avo')}, "
                f"validation time: {validation_time}, "
                f"cost: {block.transactions_info.cost if block.transactions_info else 'None'}"
                f"{percent_full_str}"
            )
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, timelord_peer: Optional[ws.WSAvocadoConnection] = None
    ) -> Optional[Message]:
    ...
    async def respond_end_of_sub_slot(
        self, request: full_node_protocol.RespondEndOfSubSlot, peer: ws.WSAvocadoConnection
    ...
    async def new_compact_vdf(self, request: full_node_protocol.NewCompactVDF, peer: ws.WSAvocadoConnection):
    ...
    async def request_compact_vdf(self, request: full_node_protocol.RequestCompactVDF, peer: ws.WSAvocadoConnection):
        header_block = await self.blockchain.get_header_block_by_height(
            request.height, request.header_hash, tx_filter=False
        )
        if header_block is None:
    ...
    async def respond_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: ws.WSAvocadoConnection):
        field_vdf = CompressibleVDFField(int(request.field_vdf))
        if not await self._can_accept_compact_proof(
            request.vdf_info, request.vdf_proof, request.height, request.header_hash, field_vdf
        ):
