...
from avocado.server.start_wallet import SERVICE_NAME
...
from avocado.util.ints import uint16, uint64
...
def print_transaction(tx: TransactionRecord, verbose: bool, name) -> None:
    if verbose:
        ...
        avocado_amount = Decimal(int(tx.amount)) / units["avocado"]
        to_address = encode_puzzle_hash(tx.to_puzzle_hash, name)
        print(f"Transaction {tx.name}")
        print(f"Status: {'Confirmed' if tx.confirmed else ('In mempool' if tx.is_in_mempool() else 'Pending')}")
        print(f"Amount: {avocado_amount} {name}")
...
async def send(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    final_amount = uint64(int(amount * units["avocado"]))
    final_fee = uint64(int(fee * units["avocado"]))
    ...
    while time.time() - start < 10:
        ...
        if len(tx.sent_to) > 0:
            ...
            print(f"Do avocado wallet get_transaction -f {fingerprint} -tx 0x{tx_id} to get status")
            return None
...
def wallet_coin_unit(typ: WalletType, address_prefix: str) -> Tuple[str, int]:
    ...
    if typ in [WalletType.STANDARD_WALLET, WalletType.POOLING_WALLET, WalletType.MULTI_SIG, WalletType.RATE_LIMITED]:
        return address_prefix, units["avocado"]
    return "", units["slice"]
...
async def get_wallet(wallet_client: WalletRpcClient, fingerprint: int = None) -> Optional[Tuple[WalletRpcClient, int]]:
    ...
    if len(fingerprints) == 0:
        print("No keys loaded. Run 'avocado keys generate' or import a key")
        return None
    ...
    if log_in_response["success"] is False:
        if log_in_response["error"] == "not_initialized":
            ...
            if "backup_path" in log_in_response:
                ...
                print(f"Backup file from backup.avocadonetwork.net downloaded and written to: {path}")
