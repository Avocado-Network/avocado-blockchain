...
def show_plots(root_path: Path):
    from avocado.plotting.plot_tools import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'avocado plots add -d [dir]' and remove with"
        + " 'avocado plots remove -d [dir]'"
        + " Scan and check plots with 'avocado plots check'"
    )
    print()
...
def plots_cmd(ctx: click.Context):
    ...
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with 'avocado init'")
...
def check_cmd(
    ...
    from avocado.plotting.check_plots import check_plots
...
def add_cmd(ctx: click.Context, final_dir: str):
    from avocado.plotting.plot_tools import add_plot_directory
...
def remove_cmd(ctx: click.Context, final_dir: str):
    from avocado.plotting.plot_tools import remove_plot_directory
