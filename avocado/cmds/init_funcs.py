...
from avocado.consensus.coinbase import create_puzzlehash_for_pk
from avocado.ssl.create_ssl import generate_ca_signed_cert, get_avocado_ca_crt_key, make_ca_cert
from avocado.util.bech32m import encode_puzzle_hash
from avocado.util.config import (
    create_default_avocado_config,
    initial_config_file,
    load_config,
    save_config,
...
from avocado.util.keychain import Keychain
from avocado.util.path import mkdir
from avocado.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'avocado keys generate'")
        return None

    config: Dict = load_config(new_root, "config.yaml")
    pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
    all_targets = []
    stop_searching_for_farmer = "avo_target_address" not in config["farmer"]
    stop_searching_for_pool = "avo_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("avo_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("avo_target_address"):
                stop_searching_for_pool = True

    # Set the destinations
    if "avo_target_address" not in config["farmer"]:
        print(f"Setting the avo destination address for coinbase fees reward to {all_targets[0]}")
        config["farmer"]["avo_target_address"] = all_targets[0]
    elif config["farmer"]["avo_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['avo_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "avo_target_address" not in config["pool"]:
        print(f"Setting the avo destination address for coinbase reward to {all_targets[0]}")
        config["pool"]["avo_target_address"] = all_targets[0]
    elif config["pool"]["avo_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['avo_target_address']} with {all_targets[0]}"
        )
...
def create_all_ssl(root: Path):
    ...
    avocado_ca_crt, avocado_ca_key = get_avocado_ca_crt_key()
    avocado_ca_crt_path = ca_dir / "avocado_ca.crt"
    avocado_ca_key_path = ca_dir / "avocado_ca.key"
    avocado_ca_crt_path.write_bytes(avocado_ca_crt)
    avocado_ca_key_path.write_bytes(avocado_ca_key)
    ...
    avocado_ca_crt, avocado_ca_key = get_avocado_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, avocado_ca_crt, avocado_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path):
    if create_certs is not None:
        if root_path.exists():
            ...
            if avocado_init(root_path) == 0 and root_path.exists():
                return init(create_certs, root_path)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return avocado_init(root_path)


def avocado_version_number() -> Tuple[str, str, str, str]:
...
def avocado_minor_release_number():
    res = int(avocado_version_number()[2])
    print(f"Install release number: {res}")
    return res


def avocado_full_version_str() -> str:
    major, minor, patch, dev = avocado_version_number()
    return f"{major}.{minor}.{patch}{dev}"


def avocado_init(root_path: Path):
    if os.environ.get("AVOCADO_ROOT", None) is not None:
        print(
            f"warning, your AVOCADO_ROOT is set to {os.environ['AVOCADO_ROOT']}. "
            f"Please unset the environment variable and run avocado init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Avocado directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if AVOCADO_ROOT is set, or if user has run avocado init twice
        # before a new update.
    ...
    check_keys(root_path)
    print("")
    print("To see your keys, run 'avocado keys show --show-mnemonic-seed'")
