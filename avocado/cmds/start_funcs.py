...
from avocado.daemon.client import DaemonProxy, connect_to_daemon_and_validate
from avocado.util.service_groups import services_for_groups
...
def launch_start_daemon(root_path: Path) -> subprocess.Popen:
    ...
    avocado = sys.argv[0]
    process = subprocess.Popen(f"{avocado} run_daemon".split(), stdout=subprocess.PIPE)
    return process
...
async def async_start(root_path: Path, group: str, restart: bool) -> None:
    ...
    if daemon is None:
        print("Failed to create the avocado daemon")
        return None
