...
from avocado.pools.pool_config import PoolWalletConfig, load_pool_config
from avocado.protocols import farmer_protocol, harvester_protocol
from avocado.protocols.pool_protocol import (
    ErrorResponse,
...
from avocado.util.bech32m import decode_puzzle_hash
from avocado.util.config import load_config, save_config, config_path_for_filename
...
from avocado.wallet.puzzles.singleton_top_layer import SINGLETON_MOD
...
class Farmer:
    def __init__(
        ...
        if len(self.get_public_keys()) == 0:
            error_str = "No keys exist. Please run 'avocado keys generate' or open the UI."
            raise RuntimeError(error_str)

        # This is the farmer configuration
        self.farmer_target_encoded = self.config["avo_target_address"]
        self.farmer_target = decode_puzzle_hash(self.farmer_target_encoded)

        self.pool_public_keys = [G1Element.from_bytes(bytes.fromhex(pk)) for pk in self.config["pool_public_keys"]]

        # This is the self pooling configuration, which is only used for original self-pooled plots
        self.pool_target_encoded = pool_config["avo_target_address"]
    ...
    def set_reward_targets(self, farmer_target_encoded: Optional[str], pool_target_encoded: Optional[str]):
        ...
        if farmer_target_encoded is not None:
            ...
            config["farmer"]["avo_target_address"] = farmer_target_encoded
        if pool_target_encoded is not None:
            self.pool_target_encoded = pool_target_encoded
            self.pool_target = decode_puzzle_hash(pool_target_encoded)
            config["pool"]["avo_target_address"] = pool_target_encoded
