...
from avocado.pools.pool_wallet_info import PoolWalletInfo
...
class WalletRpcClient(RpcClient):
    """
    Client to Avocado RPC, connects to a local wallet. Uses HTTP/JSON, and converts back from
    ...
    async def log_in(self, fingerprint: int) -> Dict:
        try:
            return await self.fetch(
                "log_in",
                {"host": "https://backup.avocadonetwork.net", "fingerprint": fingerprint, "type": "start"},
            )
    ...
    async def log_in_and_restore(self, fingerprint: int, file_path) -> Dict:
        try:
            return await self.fetch(
                "log_in",
                {
                    "host": "https://backup.avocadonetwork.net",
    ...
    async def log_in_and_skip(self, fingerprint: int) -> Dict:
        try:
            return await self.fetch(
                "log_in",
                {"host": "https://backup.avocadonetwork.net", "fingerprint": fingerprint, "type": "skip"},
            )
        except ValueError as e:
            return e.args[0]
