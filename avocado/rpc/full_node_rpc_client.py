...
from avocado.rpc.rpc_client import RpcClient
...
class FullNodeRpcClient(RpcClient):
    """
    Client to Avocado RPC, connects to a local full node. Uses HTTP/JSON, and converts back from
    JSON into native python objects before returning. All api calls use POST requests.
    Note that this is not the same as the peer protocol, or wallet protocol (which run Avocado's
