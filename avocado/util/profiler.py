...
# the output will be printed to your avocado root path, e.g. ~/.avocado/mainnet/profile/
# to analyze the profile, run:

#   python avocado/utils/profiler.py ~/.avocado/mainnet/profile | less -r

# this will print CPU usage of the avocado full node main thread at 1 second increments.
# find a time window of interest and analyze the profile file (which are in pstats format).

# for example:

#   python avocado/utils/profiler.py ~/.avocado/mainnet/profile 10 20
...
if __name__ == "__main__":
    ...
    def analyze_slot_range(profile_dir: pathlib.Path, first: int, last: int):
        ...
        output_file = "avocado-hotspot-%d" % first
        if first < last:
            output_file += "-%d" % last
