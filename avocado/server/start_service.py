...
from avocado.daemon.server import singleton, service_launch_lock_path
from avocado.server.ssl_context import avocado_ssl_ca_paths, private_ssl_ca_paths
...
class Service:
    def __init__(
        ...
        proctitle_name = f"avocado_{service_name}"
        ...
        avocado_ca_crt, avocado_ca_key = avocado_ssl_ca_paths(root_path, self.config)
        inbound_rlp = self.config.get("inbound_rate_limit_percent")
        outbound_rlp = self.config.get("outbound_rate_limit_percent")
        assert inbound_rlp and outbound_rlp
        self._server = AvocadoServer(
            advertised_port,
            node,
            peer_api,
            node_type,
    ...
    async def wait_closed(self) -> None:
        ...
        self._log.info("Waiting for AvocadoServer to be closed")
