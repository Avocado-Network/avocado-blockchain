...
async def send_all_first_reply(
    func: str, arg: Any, peers: List[WSAvocadoConnection], timeout=15
) -> Optional[Tuple[Any, WSAvocadoConnection]]:
    """performs an API request to peers and returns the result of the first response and the peer that sent it."""

    async def do_func(peer_x: WSAvocadoConnection, func_x: str, arg_x: Any):
...
async def send_to_random(func: str, arg: Any, peers: List[WSAvocadoConnection]) -> Optional[Tuple[Any, WSAvocadoConnection]]:
    """performs an API request to peers and returns the result of the first response and the peer that sent it."""
