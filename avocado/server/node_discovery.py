...
from avocado.protocols import full_node_protocol, introducer_protocol
...
class FullNodeDiscovery:
    ...
    def __init__(
        ...
        self.server: AvocadoServer = server
    ...
    async def on_connect(self, peer: ws.WSAvocadoConnection):
        if (
    ...
    async def update_peer_timestamp_on_message(self, peer: ws.WSAvocadoConnection):
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self):
        ...
        async def on_connect(peer: ws.WSAvocadoConnection):
