...
from avocado.server.introducer_peers import IntroducerPeers
from avocado.server.outbound_message import Message, NodeType
from avocado.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from avocado.util.errors import Err, ProtocolError
from avocado.util.ints import uint16
from avocado.util.network import is_localhost, is_in_network
...
class AvocadoServer:
    def __init__(
        ...
        avocado_ca_crt_key: Tuple[Path, Path],
        name: str = None,
        introducer_peers: Optional[IntroducerPeers] = None,
    ):
        # Keeps track of all connections to and from this node.
        logging.basicConfig(level=logging.DEBUG)
        self.all_connections: Dict[bytes32, WSAvocadoConnection] = {}
        self.tasks: Set[asyncio.Task] = set()

        self.connection_by_type: Dict[NodeType, Dict[bytes32, WSAvocadoConnection]] = {
            NodeType.FULL_NODE: {},
            NodeType.WALLET: {},
            NodeType.HARVESTER: {},
            NodeType.FARMER: {},
        ...
        self.avocado_ca_crt_path, self.avocado_ca_key_path = avocado_ca_crt_key
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSAvocadoConnection] = []
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.avocado_ca_crt_path, self.avocado_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
    ...
    async def incoming_connection(self, request):
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSAvocadoConnection] = None
        try:
            connection = WSAvocadoConnection(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    async def connection_added(self, connection: WSAvocadoConnection, on_connect=None):
    ...
    def connection_closed(self, connection: WSAvocadoConnection, ban_time: int):
    ...
    async def incoming_api_task(self) -> None:
        ...
        while True:
            ...
            async def api_call(full_message: Message, connection: WSAvocadoConnection, task_id):
    ...
    async def send_to_others(
        ...
        node_type: NodeType,
        origin_peer: WSAvocadoConnection,
    ):
    ...
    def get_outgoing_connections(self) -> List[WSAvocadoConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def get_full_node_outgoing_connections(self) -> List[WSAvocadoConnection]:
        result = []
    ...
    def get_full_node_connections(self) -> List[WSAvocadoConnection]:
        return list(self.connection_by_type[NodeType.FULL_NODE].values())

    def get_connections(self) -> List[WSAvocadoConnection]:
        result = []
        for _, connection in self.all_connections.items():
            result.append(connection)
        return result
    ...
    def is_trusted_peer(self, peer: WSAvocadoConnection, trusted_peers: Dict) -> bool:
