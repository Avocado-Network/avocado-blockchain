...
def get_avocado_ca_crt_key() -> Tuple[Any, Any]:
    crt = pkg_resources.resource_string(__name__, "avocado_ca.crt")
    key = pkg_resources.resource_string(__name__, "avocado_ca.key")
    return crt, key
...
def generate_ca_signed_cert(ca_crt: bytes, ca_key: bytes, cert_out: Path, key_out: Path):
    ...
    new_subject = x509.Name(
        [
            x509.NameAttribute(NameOID.COMMON_NAME, "Avocado"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Avocado"),
    ...
    cert = (
        ...
        .add_extension(
            x509.SubjectAlternativeName([x509.DNSName("avocadonetwork.net")]),
            critical=False,
        )
...
def make_ca_cert(cert_path: Path, key_path: Path):
    ...
    subject = issuer = x509.Name(
        [
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Avocado"),
            x509.NameAttribute(NameOID.COMMON_NAME, "Avocado CA"),
            x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, "Organic Farming Division"),
        ]
    )
    root_cert = (
...
def main():
    return make_ca_cert(Path("./avocado_ca.crt"), Path("./avocado_ca.key"))
